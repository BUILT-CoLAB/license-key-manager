        ...
        with app.app_context():
            ...
            try:
                DBAPI.createCustomer(
                    'Test Customer', 'test@customer.com', '123456789', 'PORTUGAL')
                DBAPI.createKey(product.id, 1, 'SDAFX-ADFAX',
                                10, int(time.time()) + 200000)
            ...
            data = {'licenseID': 1, 'idclient': '1', 'action': 'RESET',
            ...
            data = {'licenseID': 1, 'idclient': '1', 'action': 'DELETE',
                    'maxdevices': '10', 'expirydate': '1000000'}
            licenses.changeLicenseState(data)
...
def test_utils():
    ...
    with pytest.raises(Exception):
        ...
        utils.validateMultiple_Customer(newCustomer.get(
            'name'), newCustomer.get('email'), newCustomer.get('phone'))
