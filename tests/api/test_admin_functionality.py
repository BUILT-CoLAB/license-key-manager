...
def user_not_owner(app):
    with app.app_context():
        newAdmin = User(email = "testing@email.com", password =generate_password_hash("testing"), name = "notOwner", timestamp = int(time()),owner = False)
...
def test_admin_edit(auth,client,app,user_not_owner):
    ...
    response = client.post("/admins/"+str(user_not_owner.id)+"/edit",json = json_info)
